...
if True:
    ...
    vec_embeddings_reduit50 = np.load(
        "../data_midwife/vec_embeddings_sf_norm_reduit_dim50_neighbors15_mindist005.npy",
        allow_pickle=True,
    )
...
def fit_hdbscan(vec_embeddings, epsilon_table, min_table, dim=50, save=False):
    ...
    for eps in epsilon_table:
        for min in min_table:
            ...
            if save:
                np.save(
                    f"../data_midwife/labels_thg_hdbscan/labels_thg_hdbscan_dim{dim}_eps{eps}_min{min}.npy",
                    labels_np,
                )
...
if __name__ == "__main__":
    ...
    labels_test = np.load(
        "../data_midwife/table_tous_les_labels_hdbscan_dim50_eps0.005_min100.npy",
        allow_pickle=True,
    )
